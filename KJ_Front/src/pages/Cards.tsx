...
import { ChevronDown, ChevronUp, CreditCard, DollarSign, Pencil, Plus, Save, Trash2, X } from 'lucide-react';
...
export function Cards() {
  ...
  const [newCard, setNewCard] = useState({ name: '', limit: '', closingDay: '' });
  ...
  async function handleCreateCard() {
    if (!newCard.name || !newCard.closingDay) {
      await dialog.alert({
        ...
        message: 'Informe o nome do cartão, o limite e o dia de fechamento da fatura.',
      });
      return;
    }
    ...
    try {
      ...
      setNewCard({ name: '', limit: '', closingDay: '' });
      await fetchCards();
    } catch (error: any) {
      await dialog.alert({
  ...
  function updateNewCardLimit(delta: number) {
    const current = Number(newCard.limit || 0);
    setNewCard((prev) => ({ ...prev, limit: String(Math.max(0, Number((current + delta).toFixed(2)))) }));
  }
  ...
  async function handleDelete(id: string) {
    const confirmed = await dialog.confirm({
      ...
      message: 'Deseja remover este cartão? Se houver lançamentos vinculados, a exclusão será bloqueada.',
        ...
        <section className="bg-[var(--color-surface)] p-6 rounded-2xl border border-gray-800 shadow-2xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            ...
            <div className="relative">
              ...
              <input
                value={newCard.limit}
                onChange={(event) => setNewCard((prev) => ({ ...prev, limit: event.target.value }))}
                type="number"
                step="0.01"
                placeholder="Limite"
                className="no-spinner w-full bg-[var(--color-bg)] border border-gray-800 p-3 pl-9 pr-10 rounded-xl text-sm text-gray-200 outline-none focus:border-[var(--color-accent)]/50"
              />
              <div className="absolute right-2 top-1/2 -translate-y-1/2 flex flex-col">
                <button type="button" onClick={() => updateNewCardLimit(1)} className="h-4 w-5 flex items-center justify-center text-[var(--color-accent)] hover:text-[var(--color-accent-strong)]">
                  <ChevronUp size={14} />
                </button>
                <button type="button" onClick={() => updateNewCardLimit(-1)} className="h-4 w-5 flex items-center justify-center text-[var(--color-accent)] hover:text-[var(--color-accent-strong)]">
                  <ChevronDown size={14} />
                ...
                cards.map((card) => {
                  ...
                  return (
                    <div key={card.id} className="rounded-2xl border border-gray-800 bg-[var(--color-bg)] p-5">
                      <div className="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr_1fr_auto] gap-4 items-center">
                        <div>
                        ...
                        <div>
                          <p className="text-[10px] uppercase tracking-widest text-gray-500 mb-2">Limite</p>
                          {isEditing ? (
                            <div className="relative">
                              <DollarSign className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-600" size={16} />
                              <input
                                ...
                                step="0.01"
                                value={current.limit}
                                onChange={(event) => setEditingData((prev) => ({ ...prev, [card.id]: { ...current, limit: Number(event.target.value) } }))}
                                className="no-spinner w-full bg-[var(--color-surface)] border border-gray-700 p-3 pl-9 pr-10 rounded-xl text-sm text-gray-200 outline-none focus:border-[var(--color-accent)]/50"
                              />
                              <div className="absolute right-2 top-1/2 -translate-y-1/2 flex flex-col">
                                <button type="button" onClick={() => setEditingData((prev) => ({ ...prev, [card.id]: { ...current, limit: Math.max(0, Number((Number(current.limit || 0) + 1).toFixed(2))) } }))} className="h-4 w-5 flex items-center justify-center text-[var(--color-accent)] hover:text-[var(--color-accent-strong)]">
                                  <ChevronUp size={14} />
                                </button>
                                <button type="button" onClick={() => setEditingData((prev) => ({ ...prev, [card.id]: { ...current, limit: Math.max(0, Number((Number(current.limit || 0) - 1).toFixed(2))) } }))} className="h-4 w-5 flex items-center justify-center text-[var(--color-accent)] hover:text-[var(--color-accent-strong)]">
                                  <ChevronDown size={14} />
                        ...
                        <div>
                          ...
                          {isEditing ? (
                            ...
                            <p className="text-sm font-semibold text-rose-300">
                              {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(card.outstandingAmount || 0)}
                            </p>
                          )}
                        </div>
